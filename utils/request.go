...
func HTTPRequest(request Request, isAllResponse bool, ctxs []*gin.Context) (map[string]interface{}, error) {
	...
	if RequestIsTracing && len(ctxs) > 0 {
		ctx := ctxs[0]
		tracer, IsExist := ctx.Get("Tracer")
		if !IsExist {
			return resData, errors.New("Tracer is not exist")
		}

		parentSpanContext, IsExist := ctx.Get("ParentSpanContext")
		if !IsExist {
			return resData, errors.New("ParentSpanContext is not exist")
		}
